...
float get_vel_steering(float streer_angle, int flag){
    ...
    if(flag == 1 || flag == 2 ){
        ...
        if(angle_for_Ardu < -30) angle_for_Ardu = CENTER_STEER - CENTER_PLUS2; // 오른쪽으로 가야함
...
void scanCallback(const sensor_msgs::LaserScan::ConstPtr& scan)
{
    ...
    int flag = 1; // 4가지 케이스(0.straight 1.slowly 2.urgently 3.wall) 중 어떤 케이스인지 저장.
    float streer_angle = 0; // 자동차가 틀어야 하는 각도

    float dis_diff = (left_dist < right_dist)? right_dist - left_dist : left_dist - right_dist ;    
    if (dis_diff < THRESHOLD_DIS_DIFF1){  // 직진 하도록
        streer_angle = theta - 90;
        flag = 1;
        ROS_INFO("CASE 11111111");
    }
    else if(dis_diff>= THRESHOLD_DIS_DIFF1 && dis_diff < THRESHOLD_DIS_DIFF2){   // 중앙으로 가도록. 심각성 중
        ...
        flag = 2;
        ROS_INFO("CASE 22222222");
    }
    else if(dis_diff>= THRESHOLD_DIS_DIFF2 && dis_diff < THRESHOLD_DIS_DIFF3){   // 중앙으로 가도록. 심각성 상
        if (left_dist < right_dist){
